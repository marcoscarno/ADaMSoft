...
/**
* This is the procedure that replace a missing data value with mean taken from the not missing values
* @author marco.scarno@gmail.com
* @date 14/02/2017
*/
public class ProcMdkmeans extends ObjectTransformer implements RunStep
{
	/**
	* Starts the execution of Proc Mdkmeans
	*/
	...
	public Result executionresult(Hashtable<String, Object> parameters)
	{
		...
		String keyword="MdKMeans "+dict.getkeyword();
		String description="MdKMeans "+dict.getdescription();
		...
		double[] valseeds=new double[usedvar.length];
				...
				else
				{
					...
					else
					{
						...
						for(int i=0; i<nseeds; i++)
						{
							...
							distances[i]=ed.getdistance(valseeds, realval);
							if (distances[i]<min)
							{
								min=distances[i];
								pointermin=i;
									...
									if (totalvar[i].equals(usedvar[j]))
									{
										...
										if (Double.isNaN(testmd))
										{
											if (!wwp)
											{
												...
												double vals=(valse.get(usedvar[j])).doubleValue();
											...
											else
											{
												...
												for (int k=0; k<tempseed.size(); k++)
												{
													...
													double tempvals=(valse.get(usedvar[j])).doubleValue();
													vals+=tempvals*distances[k];
													sumvals+=distances[k];
												}
												vals=vals/sumvals;
												newval[i]=double2String(vals);
	...
	*/
	public LinkedList<GetRequiredParameters> getparameters()
	{
		...
		parameters.add(new GetRequiredParameters(Keywords.weight, "checkbox", false, 1232, dep, "", 2));
