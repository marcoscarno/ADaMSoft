...
/**
* This is the procedure that evaluate the probability of the statistical test for correlations between row and column variables
* @author marco.scarno@gmail.com
* @date 13/02/2017
*/
public class ProcCorrprob extends ObjectTransformer implements RunStep
{
	/**
	* Starts the execution of Proc Corrtest
	*/
	public Result executionresult(Hashtable<String, Object> parameters)
	{
		...
		String keyword="Corrtest "+dict.getkeyword();
		String description="Corrtest "+dict.getdescription();
		...
		CorrelationsEvaluator ec=new CorrelationsEvaluator(pairwise, samevars);
		NValidEvaluator nve=new NValidEvaluator(pairwise);
		int validgroup=0;
		String[] values=null;
		...
		while (!data.isLast())
		{
			...
			if (values!=null)
			{
				...
				if ((vp.vargroupisnotmissing(vargroupvalues)) && (!Double.isNaN(weightvalue)))
				{
					...
					nve.setValue(vargroupvalues, varrowvalues, varcolvalues, weightvalue);
				}
			}
		}
		data.close();
		...
		Hashtable<Vector<String>, double[][]> nv=nve.getresult();
		...
		for (int i=0; i<totalgroupmodalities; i++)
		{
			...
			double[][] tempnv=nv.get(rifmodgroup);
			for (int j=0; j<tempcorr.length; j++)
			{
				...
				for (int k=0; k<tempcorr[0].length; k++)
				{
					...
					try
					{
						tval=Math.sqrt((tempnv[j][k]-2)/(1-Math.pow(tempcorr[j][k],2)));
						tval=tval*tempcorr[j][k];
						tval=Math.abs(tval);
						tval=(1-(Probability.studentT(tempnv[j][k]-2,tval)))*2;
					}
					catch (Exception e) {}
					valuestowrite[varg.length+1+k]=double2String(tval);
				}
