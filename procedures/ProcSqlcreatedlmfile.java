...
/**
* This procedure executes a query and loads the result into an ASCII delimited file
* @author marco.scarno@gmail.com
* @date 14/02/2017
*/
public class ProcSqlcreatedlmfile implements RunStep
{
	/**
	...
	*/
	...
	{
		String mess="";
		String [] requiredparameters=new String[] {Keywords.table, Keywords.dlmfile};
		...
		String dlmfile=(String)parameters.get(Keywords.dlmfile);
		...
		if (!dlmfile.toLowerCase().endsWith(".txt"))
			dlmfile=dlmfile+".txt";

		BufferedWriter fileouttxt=null;
		boolean exist=(new File(dlmfile)).exists();
		if (exist)
		{
			boolean success = (new File(dlmfile)).delete();
			if (!success)
				return new Result("%2744%<br>\n", false, null);
		}
		try
		{
			fileouttxt = new BufferedWriter(new FileWriter(dlmfile, true));
		}
		catch (Exception e)
		{
			return new Result("%2745% ("+dlmfile+")<br>\n", false, null);
		}
		...
		{
			fileouttxt.close();
			return new Result("%2746% ("+dlmfile+")<br>\n", true, null);
		}
		catch (Exception e)
		{
			return new Result("%2747% ("+dlmfile+")<br>\n"+e.toString()+"<br>", false, null);
		}
	}
	public LinkedList<GetRequiredParameters> getparameters()
	{
		...
		parameters.add(new GetRequiredParameters(Keywords.dlmfile, "filesave=.txt", true, 2742, dep, "", 2));
