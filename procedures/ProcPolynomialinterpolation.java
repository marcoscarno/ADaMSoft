...
/**
* This is the procedure that interpolate with a Polynomial a series of data
* @author marco.scarno@gmail.com
* @date 17/10/2017
*/
public class ProcPolynomialinterpolation extends ObjectTransformer implements RunStep
{
	/**
	...
	*/
	public Result executionresult(Hashtable<String, Object> parameters)
	{
		...
		String [] requiredparameters=new String[] {Keywords.dictxy, Keywords.dictxint, Keywords.varx, Keywords.varxint, Keywords.vary, Keywords.degree, Keywords.OUT.toLowerCase()};
		String [] optionalparameters=new String[] {Keywords.OUTEST.toLowerCase(), Keywords.where, Keywords.weight, Keywords.replace};
		...
		String ndegree=(String)parameters.get(Keywords.degree.toLowerCase());
		int degree=string2int(ndegree);
		if (degree<2)
			return new Result("%4205%<br>\n", false, null);
		...
		VariableUtilities varu=new VariableUtilities(dictxy, null, null, weight, tempvarx, tempvary);
		if (varu.geterror())
			return new Result(varu.getmessage(), false, null);

		String keyword="Polynomialinterpolation "+dictxy.getkeyword();
		String description="Polynomialinterpolation "+dictxy.getdescription();
		String edescription="Polynomialinterpolation: parameters estimation "+dictxy.getdescription();
		...
		try
		{
			for (int i=0; i<obs_vect.size(); i++)
			{
				...
				double[] temp_coeff = fitter.fit(obs_vect.get(i).toList());
				int diffent=0;
		...
		if (isoute)
		{
			...
			{
				dsue.addnewvar("coeff_degree_"+String.valueOf(i), "%4210% "+String.valueOf(i), Keywords.NUMSuffix, clvar, tempmd);
			}
			if (!dwe.opendatatable(dsue.getfinalvarinfo()))
				return new Result(dwe.getmessage(), false, null);
			String[] values_est=new String[degree+2];
			for (int i=0; i<vary.length; i++)
			{
				values_est[0]=vary[i];
				...
				try
				{
					...
					if (!Double.isNaN(xd))
					{
						...
						for (int i=0; i<vary.length; i++)
						{
							...
							for (int j=0; j<temp_coeff.length; j++)
							{
								currval=currval+Math.pow(xd,j)*temp_coeff[j];
							}
	...
	*/
	public LinkedList<GetRequiredParameters> getparameters()
	{
		...
		parameters.add(new GetRequiredParameters(Keywords.weight, "var=all", false, 656, depa, "", 2));
		parameters.add(new GetRequiredParameters(Keywords.degree,"text", true, 4220,dep,"",2));
