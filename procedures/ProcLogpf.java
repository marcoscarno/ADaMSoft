...
/**
* This is the procedure that estimates the weight for each unit according to the rim-weighting vincolated logaritmic function
* @author marco.scarno@gmail.com
* @date 14/02/2017
*/
public class ProcLogpf extends ObjectTransformer implements RunStep
{
	/**
	* Starts the execution of Proc Logpf
	*/
	...
	public Result executionresult(Hashtable<String, Object> parameters)
	{
		...
		Keywords.variniweight, Keywords.popdim, Keywords.where, Keywords.replace, Keywords.weightname, Keywords.todisk, Keywords.iterations, Keywords.accuracy,
		Keywords.tablewithfreq, Keywords.vargroupinuniverse, Keywords.novgconvert};
				...
				if (values!=null)
				{
					...
					if (vp.vargroupisnotmissing(vargroupvalues))
					{
						...
						if (lambda.get(vargroupvalues)==null)
						{
							...
							for (int i=0; i<rifobs; i++)
							{
								fxxt.write(vargroupvalues, actualv);
							}
						...
						if (pipok)
						{
							...
							double[] tef=new double[0];
							if (iter==0)
								tef=ef.get(vargroupvalues);
							...
							double[] ttactualv=new double[rifobs];
							for (int i=0; i<rifobs; i++)
							{
								ttactualv[i]=0;
								tempq=0;
								if (actualv[i]!=0)
								{
									...
									tempq=Math.exp(q*tempq);
									for (int j=0; j<rifobs; j++)
									{
										if (actualv[j]!=0)
										{
											tempd=fxxt.read(vargroupvalues, i, j);
											tempd=tempd+actualv[i]*actualv[j]*q*d*tempq;
											if (Double.isNaN(tempd))
												algerr=true;
											else if (Double.isInfinite(tempd))
												algerr=true;
											fxxt.write(vargroupvalues, tempd, i, j);
										}
									}
								}
								ttactualv[i]=tactualv[i]+actualv[i]*d*(tempq-1);
							}
							fxxt.deassignbasefile();
							dxexpqxlm1.put(vargroupvalues, ttactualv);
						}
					}
				}
			}
					...
					if (pipok)
					{
						...
						for (int i=0; i<vars.length; i++)
						{
							if (!values[vars[i]].equals(""))
							{
								...
								weightvalue=weightvalue+templambda[posvn]*actualv[posvn];
							}
						}
						weightvalue=d*(Math.exp(q*weightvalue));
					}
		...
		String keyword="Logpf "+dict.getkeyword();
		String description="Logpf "+dict.getdescription();
