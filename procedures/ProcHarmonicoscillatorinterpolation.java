...
/**
* This is the procedure that interpolate with a Harmonic Oscillator a series of data
* @author marco.scarno@gmail.com
* @date 17/10/2017
*/
public class ProcHarmonicoscillatorinterpolation extends ObjectTransformer implements RunStep
{
	/**
	...
	*/
	public Result executionresult(Hashtable<String, Object> parameters)
	{
		...
		String edescription="Harmonic Oscillator interpolation: parameters estimation "+dictxy.getdescription();
		...
		try
		{
			...
			{
				double[] temp_coeff = HarmonicCurveFitter.create().fit(obs_vect.get(i).toList());
				int diffent=0;
		...
		if (isoute)
		{
			...
			dsue.addnewvar("coeff_amplitude", "%4225% ", Keywords.NUMSuffix, clvar, tempmd);
			dsue.addnewvar("coeff_ang_freq", "%4226% ", Keywords.NUMSuffix, clvar, tempmd);
			dsue.addnewvar("coeff_phase", "%4227% ", Keywords.NUMSuffix, clvar, tempmd);
				...
				try
				{
					...
					if (!Double.isNaN(xd))
					{
						...
						for (int i=0; i<vary.length; i++)
						{
							...
							try
							{
								HarmonicOscillator.Parametric gp=new HarmonicOscillator.Parametric();
