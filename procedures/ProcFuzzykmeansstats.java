...
/**
* This is the procedure that evaluates several statistics for the fuzzy k means
* @author marco.scarno@gmail.com
* @date 14/02/2017
*/
public class ProcFuzzykmeansstats extends ObjectTransformer implements RunStep
{
	/**
	* Starts the execution of Proc Fuzzykmeansstats
	*/
	public Result executionresult(Hashtable<String, Object> parameters)
	{
		...
		String keyword="Fuzzykmeansstats "+dict.getkeyword();
		String description="Fuzzykmeansstats "+dict.getdescription();
		...
		{
			dsu.addnewvarfromolddict(dict, groupvar[j], temph, temph, "g_"+String.valueOf(j));
		}

		dsu.addnewvar("Dunn1", "%1153%", Keywords.NUMSuffix, temph, temph);
		dsu.addnewvar("Dunn2", "%1154%", Keywords.NUMSuffix, temph, temph);
		dsu.addnewvar("Entropy", "%1155%", Keywords.NUMSuffix, temph, temph);
		dsu.addnewvar("Windham", "%1156%", Keywords.NUMSuffix, temph, temph);
		...
		while ((!data.isLast()) && (noterror))
		{
			...
			if (values!=null)
			{
				...
				{
					validgroup++;
					noterror=efuzzykmeans.addstat(vargroupvalues, realval);
				}
			}
		}
		data.close();
		...
		Hashtable<Vector<String>, double[]> stat=efuzzykmeans.getstat();
		efuzzykmeans.endfuzzykmeans();

		for (Enumeration<Vector<String>> e = stat.keys() ; e.hasMoreElements() ;)
		{
			values=new String[groupvar.length+4];
			Vector<String> gv=e.nextElement();
			double[] tempstat=stat.get(gv);
			for (int i=0; i<gv.size(); i++)
			{
				String tempval=gv.get(i);
				if (tempval!=null)
					values[i]=tempval;
			}
			values[groupvar.length]=double2String(tempstat[0]);
			values[groupvar.length+1]=double2String(tempstat[1]);
			values[groupvar.length+2]=double2String(tempstat[2]);
			values[groupvar.length+3]=double2String(tempstat[3]);
			dw.write(values);
		}
	...
	*/
	public LinkedList<GetRequiredParameters> getparameters()
	{
		...
		parameters.add(new GetRequiredParameters(Keywords.OUT.toLowerCase()+"=", "setting=out", true, 1230, dep, "", 1));
