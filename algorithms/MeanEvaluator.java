...
/**
* This method evaluates the means for several variables, also according do the values of the grouping variables
...
public class MeanEvaluator
{
	...
	/**
	*Initialise the main Objects, that will contains the means
	*/
	...
	*/
	public void setValue(Vector<String> groupval, double[] val, double w)
	{
		...
		double[] valid=new double[val.length];
		if (test==null)
		{
	...
	*/
	public void calculate()
	{
		for (Enumeration<Vector<String>> e = mean.keys() ; e.hasMoreElements() ;)
		{
			...
			double[] num =n.get(gv);
			double[] meanval=new double[v.length];
			for (int i=0; i<v.length; i++)
			{
				meanval[i]=Double.NaN;
				try
				{
					meanval[i]=v[i]/num[i];
				}
