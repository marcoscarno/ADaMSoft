...
/**
* This method evaluates the corrected sum of squares for several variables, also according do the values of the grouping variables
* @author marco.scarno@gmail.com
* @date 13/02/2017
*/
public class SSQEvaluator
{
	...
	*/
	...
	/**
	*Finalizes the corrected sum of squares
	*/
	public void calculate()
	{
		for (Enumeration<Vector<String>> e = mean.keys() ; e.hasMoreElements() ;)
		{
			...
			for (int i=0; i<v.length; i++)
			{
				...
				try
				{
					stdval[i]=num[i]*((s[i]/num[i])-(v[i]/num[i])*(v[i]/num[i]));
				}
				catch (Exception ex) {}
			}
			ssq.put(gv, stdval);
	...
	/**
	*Gives back the corrected sum of squares
	*/
